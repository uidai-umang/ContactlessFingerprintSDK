...
class FingerDetector:
    ...
    def texture_check(self, roi):
        ...
        if roi.size == 0 or roi.shape[0] < 5 or roi.shape[1] < 5:
            print("[FingerDetect] ROI too small for texture analysis")
            return 0.0, 0.0

        gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)

        # Laplacian variance (high‑freq energy)
        ...
        print(f"[FingerDetect] LapVar: {lap_var:.2f} | EdgeDensity: {edge_density:.2f}% | StdDev: {float(stddev[0][0]):.2f}")
    ...
    def process(self, nv21_bytes, width, height):
        ...
        print(f"[FingerDetect] Area Ratio: {area_ratio:.3f} | Area: {area}")
        ...
        box = [x, y, w, h]
        roi = bgr_frame[y:y+h, x:x+w]  # take ROI from original frame for texture

        # Texture gate (hybrid)
        lap_passed, edge_passed = self.texture_check(roi)
        if not lap_passed or not edge_passed:
           print("[FingerDetect] Texture check FAILED")
...
def main(
    ...
    # Update the detector's thresholds before processing
